...
import shutil
import inspect
from rpyc_async.lib.compat import pickle, execute
from rpyc_async.core.service import ClassicService, Slave
from rpyc_async.utils import factory
from rpyc_async.core.service import ModuleNamespace  # noqa: F401
from rpyc_async.core.consts import STREAM_CHUNK
...
FORK_SERVER_COMMAND = "rpyc-async-classic"


def _resolve_server_file(server_file=None, _which=None):
    """Resolve the classic-server script to launch.

    An explicit ``server_file`` always wins. Otherwise only the installed entry
    point ``rpyc-async-classic`` is considered -- never an upstream
    ``rpyc_classic.py``, which would silently connect incompatible peers.

    :param _which: injection point for tests; defaults to :func:`shutil.which`.
    """
    if server_file is not None:
        return server_file
    which = _which if _which is not None else shutil.which
    found = which(FORK_SERVER_COMMAND)
    if not found:
        raise ValueError(
            f"server_file not given and {FORK_SERVER_COMMAND!r} was not found on PATH"
        )
    return found
...
def connect_subproc(server_file=None):
    ...
    :param server_file: The full path to the server script. If not given, the
                        installed ``rpyc-async-classic`` command is located on
                        ``PATH``. In a source checkout, pass
                        ``bin/rpyc_async_classic.py`` explicitly.

    :returns: an RPyC connection exposing ``SlaveService``
    """
    server_file = _resolve_server_file(server_file)
...
def deliver(conn, localobj):
    ...
    **by value**. changes made to the remote object will not reflect locally.
    ...
    return conn.modules["rpyc_async.lib.compat"].pickle.loads(
...
def teleport_function(conn, func, globals=None, def_=True):
    ...
    from rpyc_async.utils.teleportation import export_function
    exported = export_function(func)
    return conn.modules["rpyc_async.utils.teleportation"].import_function(
