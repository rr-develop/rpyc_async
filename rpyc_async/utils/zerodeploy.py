...
import rpyc_async.utils.factory
import rpyc_async.utils.classic
try:
...
class DeployedServer(object):
    ...
    def __init__(self,
                 remote_machine,
                 server_class="rpyc_async.utils.server.ThreadedServer",
                 service_class="rpyc_async.core.service.SlaveService",
        ...
        rpyc_root = local.path(rpyc_async.__file__).up()
        self._tmpdir_ctx = remote_machine.tempdir()
        tmp = self._tmpdir_ctx.__enter__()
        copy(rpyc_root, tmp / "rpyc_async")
        ...
        if hasattr(remote_machine, "connect_sock"):
            ...
            self.local_port = rpyc_async.utils.factory._get_free_port()
...
class MultiServerDeployment(object):
    ...
    def __init__(self, remote_machines, server_class="rpyc_async.utils.server.ThreadedServer"):
